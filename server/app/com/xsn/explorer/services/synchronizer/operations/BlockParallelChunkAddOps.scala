...
class BlockParallelChunkAddOps @Inject()(blockChunkRepository: BlockChunkRepository.FutureImpl)(
  ...
  private def storeBlockData(
    ...
    val result = for {
      ...
      _ <- linkF.toFutureOr
      _ = logger.info(s"Syncing block data took ${System.currentTimeMillis() - start} ms")
      _ <- blockChunkRepository.upsertSyncState(block.hash, nextState).toFutureOr
      _ <- continueFromState(nextState, block, tposContracts).toFutureOr
    } yield ()
  ...
  private def storeTransactions(
    ...
    val result = for {
      ...
      _ = logger.info(s"Syncing transactions took ${System.currentTimeMillis() - start} ms")
      _ <- blockChunkRepository.upsertSyncState(block.hash, nextState).toFutureOr
      _ <- continueFromState(nextState, block, tposContracts).toFutureOr
    } yield ()
  ...
  private def storeOutputs(
    ...
    val result = for {
      ...
      _ = logger.info(s"Syncing outputs took ${System.currentTimeMillis() - start} ms")
      _ <- blockChunkRepository.upsertSyncState(block.hash, nextState).toFutureOr
      _ <- continueFromState(nextState, block, tposContracts).toFutureOr
    } yield ()
  ...
  private def storeInputs(
    ...
    val result = for {
      ...
      _ = logger.info(s"Syncing inputs took ${System.currentTimeMillis() - start} ms")
      _ <- blockChunkRepository.upsertSyncState(block.hash, nextState).toFutureOr
      _ <- continueFromState(nextState, block, tposContracts).toFutureOr
    } yield ()
  ...
  private def spendOutputs(
    ...
    val result = for {
      ...
      _ = logger.info(s"Spending outputs took ${System.currentTimeMillis() - start} ms")
      _ <- blockChunkRepository.upsertSyncState(block.hash, nextState).toFutureOr
      _ <- continueFromState(nextState, block, tposContracts).toFutureOr
    } yield ()
  ...
  private def storeAddressTransactionDetails(
    ...
    val result = for {
      ...
      _ = logger.info(s"Syncing address details took ${System.currentTimeMillis() - start} ms")
      _ <- blockChunkRepository.upsertSyncState(block.hash, nextState).toFutureOr
      _ <- continueFromState(nextState, block, tposContracts).toFutureOr
    } yield ()
  ...
  private def updateBalances(block: Block.HasTransactions): FutureApplicationResult[Unit] = {
    ...
    val result = blockChunkRepository.atomicUpdateBalances(block.hash, balanceList)
    result.foreach { _ =>
      logger.info(s"Updating balances took ${System.currentTimeMillis() - start} ms")
    }
    result
  }
}
